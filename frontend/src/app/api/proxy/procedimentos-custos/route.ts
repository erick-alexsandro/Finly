...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!procedimentoId) {
      return NextResponse.json({ error: "Missing procedimentoId parameter" }, { status: 400 });
    }
    const backendUrl = `${process.env.BACKEND_URL || "http://localhost:8080"}/api/procedimentos/${procedimentoId}/custos`;
    ...
    console.error("[procedimentos-custos GET]", error);
...
export async function PUT(req: NextRequest) {
  try {
    ...
    console.error("[procedimentos-custos PUT]", error);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
